const localdontfs = require('node:fs');
const wasmBuffer = localdontfs.readFileSync('msieve.wasm');
let wasmfactormodule = null
...
var DYNAMIC_BASE = 5466768,
    DYNAMICTOP_PTR = 223728;
...
function createWasm() {
    ...
    function instantiateArrayBuffer(receiver) {
        return getBinaryPromise().then(function(binary) {
            console.log('prepare wasm binary')
            wasmfactormodule = WebAssembly.instantiate(binary, info)
            return wasmfactormodule
...
var ___tm_current = 223744;
var ___tm_timezone = (stringToUTF8("GMT", 223792, 4), 223792);
...
function _publishFactor(factor_type, factor_number) {
    // console.log(factor_type, UTF8ToString(factor_number))
    Module.publishFactor(factor_type, UTF8ToString(factor_number))
}

function _publishInputNumber(input_number) {
    // console.log(UTF8ToString(input_number))
    Module.publishInputNumber(UTF8ToString(input_number))
}

function _publishState(num_relations, full_relations, combined_relations, partial_relations, max_relations) {
...
var _sieve = Module["_sieve"] = function() {
...
async function factorize(n) {
  ...
  msieve.publishState = function (numRelations, fullRelations, combinedRelations, partialRelations, maxRelations) {
    // console.log({ type: 'state', numRelations, fullRelations, combinedRelations, partialRelations, maxRelations });
  };
  msieve.publishFactor = function (factorType, factorNumber) {
    // console.log({ type: 'factor', factorType, factorNumber });
    factors.push(BigInt(factorNumber))
  };
  msieve.publishInputNumber = function (inputNumber) {
    // console.log({ type: 'input', inputNumber });
  };

  const result = msieve.ccall("sieve", 'number', ['string'], [n.toString()]);
  // console.log({ type: 'finish', result });
  return factors
}

const divisors = async (integer) => {
  ...
  const list = await factorize(integer)
...
const s = async (integer) => sum(await divisors(integer))

const digits = (integer) => integer.toString().length

const aliquot = async (integer) => {
  let steps = 0
  const previous = []
  let value = integer
  ...
  while (true) {
    ...
    console.log(steps, value, digits(value), currentTime-lastTime, 'ms', Math.floor((currentTime-startTime)/36000)/100, 'h')
